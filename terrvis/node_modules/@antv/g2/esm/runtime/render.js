...
export function render(options, context = {}, resolve, reject) {
    // Initialize the context if it is not provided.
    const { width = 640, height = 480, theme } = options;
    if (!theme) {
        error('ChartOptions.theme is required, such as `const chart = new Chart({ theme: "classic"})`.');
    }
...
export function renderToMountedElement(options, context = {}, resolve, reject) {
    // Initialize the context if it is not provided.
