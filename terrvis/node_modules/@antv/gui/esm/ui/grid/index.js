...
import { classNames, distance, getCallbackValue, select, getPrimitiveAttributes, } from '../../util';
...
function renderGridLine(container, data, attr, style) {
    ...
    return container
        ...
        return enter.append('path').each(function (datum, index) {
            var lineStyle = getCallbackValue(getPrimitiveAttributes(__assign({ path: datum.path }, style)), [
                datum,
                index,
                lines,
            ]);
            this.attr(__assign({ class: CLASS_NAMES.line.name, stroke: '#D9D9D9', lineWidth: 1, lineDash: [4, 4] }, lineStyle));
        });
    }, function (update) {
