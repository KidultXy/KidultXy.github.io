...
import { degToRad, keyframeInterpolate, normalize, omit, renderExtDo, scaleToPixel, subStyleProps, vertical, } from '../../../util';
import { CLASS_NAMES } from '../constant';
import { baseDependencies } from './utils';
export var getLineAngle = memoize(function (value, attr) {
    var startAngle = attr.startAngle, endAngle = attr.endAngle;
    return (endAngle - startAngle) * value + startAngle;
}, function (value, attr) { return [value, attr.startAngle, attr.endAngle].join(); });
export var getLineTangentVector = memoize(function (value, attr) {
    if (attr.type === 'linear') {
    ...
    var dependencies = baseDependencies(attr);
...
export var getLinearValuePos = memoize(function (value, attr) {
...
}, function (value, attr) { return __spreadArray(__spreadArray([value], __read(attr.startPos), false), __read(attr.endPos), false).join(); });
export var getArcValuePos = memoize(function (value, attr) {
    ...
    return __spreadArray([value, startAngle, endAngle, radius], __read(center), false).join();
});
